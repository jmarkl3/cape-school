...
function CourseElement(props) {
    ...
    useEffect(()=>{
        ...
        if(props.mode === "view")
            setStep(props.getElementStep(props.chapterId, props.sectionId, props.elementData.id))

        // If there is a correct index get it from element data
        var tempCorrectIndex = props.elementData.correctIndex
        if(tempCorrectIndex != null && tempCorrectIndex != undefined)
            setCorrectIndex(tempCorrectIndex)        
        else
            setCorrectIndex(0)
        
    }, [props.sectionId, props.chapterId])

    function textConditional(content, index){
        ...
        if(props.mode === "edit")
            return props.elementData.content.map((content, index)=>(                
                <div className='questionAnswer' key={"answer"+key+index}>
                    ...
                    <div key={key+index+"correctInput"+forceRefresh} onClick={()=>setCorrectIndex(index)} className={"questionRadioButon "+(correctIndex == index && " selectedRadioButton")} >Correct</div>
                </div>
            ))
        else
            return props.elementData.content.map((content, index)=>(
    ...
    // Sets of buttons to display based on props.mode    
    ...
    function editButtons(_index){
        ...
        buttonsArray.push(<div className='button' onClick={()=>props.deleteElement(props.chapterId, props.sectionId, props.elementData.id)}>Delete</div>)
        buttonsArray.push(<div className='button' onClick={()=>props.addContent(props.chapterId, props.sectionId, props.elementData.id)}>Add Content</div>)
    ...
    function logType(){
        console.log(document.getElementById("typeSelect"+props.elementData.id).value)
        updateElement()
    }
